...
result_t StringDecoder_base::_new(exlib::string data,
    obj_ptr<StringDecoder_base>& retVal, v8::Local<v8::Object> This)
{
    exlib::string new_encoding = normalizeEncoding(data);
    bool isValid = false;
    Buffer_base::isEncoding(new_encoding, isValid);
    if (!isValid) {
        return CHECK_ERROR(Runtime::setError("Unknown encoding: " + data));
    }
...
result_t StringDecoder::utf16Text(Buffer_base* buf, int32_t i, exlib::string& retVal)
{
    int32_t bufLen;
    int32_t b;
    ...
    buf->_indexed_getter(bufLen - 1, b);
    m_lastChar->_indexed_setter(0, b);
    buf->toString("utf16le", i, bufLen - 1, retVal);
...
result_t StringDecoder::base64Text(Buffer_base* buf, int32_t i, exlib::string& retVal)
{
    int32_t bufLen;
    int32_t b;
    buf->get_length(bufLen);
    int32_t n = (bufLen - i) % 3;

    if (n == 0) {
        buf->toString("base64", i, -1, retVal);
        return 0;
    }
    m_lastNeed = 3 - n;
    m_lastTotal = 3;
    ...
    buf->toString("base64", i, bufLen - n, retVal);
    return 0;
}

result_t StringDecoder::utf8Text(Buffer_base* buf, int32_t i, exlib::string& retVal)
{
    int32_t total = utf8CheckIncomplete(this, buf, i);
    if (m_lastNeed == 0) {
        buf->toString("utf8", i, -1, retVal);
        return 0;
    }
    ...
    buf->toString("utf8", i, end, retVal);
    return 0;
}
...
{
    buf->toString(this->m_encoding, 0, -1, retVal);
    return 0;
}
