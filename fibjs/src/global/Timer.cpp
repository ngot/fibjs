...
public:
    JSTimer(v8::Local<v8::Function> callback, OptArgs* args,
            int32_t timeout = 0, bool repeat = false)
    ...
    // JSTimer(v8::Local<v8::Function> callback)
    //     : Timer(0, false)
    //     , m_clear_pendding(false)
    // {
    //     Isolate* isolate = holder();
    ...
    double timeout, OptArgs args, obj_ptr<Timer_base>& retVal)
{
    ...
    obj_ptr<Timer> timer = new JSTimer(callback, &args, timeout, true);
    timer->sleep();
    retVal = timer;
    ...
    double timeout, OptArgs args, obj_ptr<Timer_base>& retVal)
{
    ...
    obj_ptr<Timer> timer = new JSTimer(callback, &args, timeout);
    timer->sleep();
    retVal = timer;
